...
@onready var text_box := %Speech
@onready var timer := $LineTimer
@onready var speaker_box := %Speaker
...
func _input(event: InputEvent) -> void:
	if (event.is_action_pressed("interact") or event.is_action_pressed("fire")) and visible: 
